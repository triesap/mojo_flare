...
|---|---|
| Malformed HTTP/1.1 request (header injection, smuggling, CRLF). | `flare.http.proto` is a sans-I/O parser with RFC 7230 / 9112 token validation. CR / LF / NUL rejected at parse time; multiple `Content-Length` rejected by the strict default (the named opt-in flag is `_ExperimentalH1LeniencyConfig.allow_multiple_content_length`, off by default). TE-chunked-with-CL rejected. Conformance suite under `conformance/h1/` covers the negative cases. The named relaxations on `_ExperimentalH1LeniencyConfig` are public contract surface today; the per-flag parser plumbing for everything beyond `allow_lf_only_line_endings` and `allow_obs_fold` lands in v0.9. The strict default (every flag off) is fully wired and is the production-safe pick. |
